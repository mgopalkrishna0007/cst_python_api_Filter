...
#     f0 = 12.50  # Center frequency in Hz
...
def calculate_s21_te(freq_array, f0=inputparameters[0], FBW=0.03, N=1, Lr_dB=-30, return_all=False):
    """
    Calculate filter S-parameters for given frequency array.
    
    Parameters:
    -----------
    freq_array : numpy.ndarray
        Array of frequencies (Hz) at which to calculate filter response
    f0 : float, optional
        Center frequency (Hz). Default: 28e9
    FBW : float, optional
        Fractional bandwidth. Default: 0.03
    N : int, optional
        Filter order. Default: 1
    Lr_dB : float, optional
        Reflection coefficient in dB. Default: -30
    return_all : bool, optional
        If True, return all calculated parameters. Default: False
    
    Returns:
    --------
    S11, S21 : numpy.ndarray
        Complex S-parameters at the input frequencies
    If return_all=True, also returns:
        f : frequency array
        g : prototype elements
        M_coupling : coupling matrix
        Qe1, Qen : external Q factors
    """
    
    f = np.asarray(freq_array)
    BW = f0 * FBW
    
    # Calculate passband ripple from Lr
    ...
    if return_all:
        return S11, S21, f, g, M_coupling, Qe1, Qen
    else:
...
def calculate_pcr(matrix, inputparameters): 
    ...
    try:
        ...
        myCST.Solver.SetNumberOfResultDataSamples(1001)
        ...
        te = 1 - (((abs(SZMin1ZMax1))**2) / denominator)
