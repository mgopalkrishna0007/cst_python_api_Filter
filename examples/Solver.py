...
class Solver:
    ...
    def SetNumberOfResultDataSamples(self, nSamples: int):
        """
        Sets the number of result data samples in the CST project.
        This affects the resolution or sampling density used when evaluating
        results like farfield plots or S-parameters.
        Parameters
        ----------
        nSamples : int
            Number of samples to be used. Must be greater than zero.
        Raises
        ------
        TypeError
            If nSamples is not of type int.
        ValueError
            If nSamples is not greater than zero.
        RuntimeError
            If the VBA code is not executed successfully by CST.
        """
        # Validate input type
        if not isinstance(nSamples, int):
            raise TypeError("ERROR: nSamples must be of type int.")
        # Validate value
        if nSamples <= 0:
            raise ValueError("ERROR: nSamples must be greater than zero.")
        # Construct VBA command
        vba = (
            'With FDSolver\n'
            f'    .SetNumberOfResultDataSamples "{nSamples}"\n'
            'End With'
        )
        # Send the VBA command to CST
        self.__MWS._FlagAsMethod("AddToHistory")
        result = self.__MWS.AddToHistory("Set Result Data Samples", vba)
        # Raise error if command fails
        if result != True:
            raise RuntimeError(
                "ERROR: Execution of the VBA code to set number of result data samples failed."
            )
        return
