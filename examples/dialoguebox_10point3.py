# filter code optimization , extension of dialoguebox_10point2.py , but with linear decay of inertia
...
inputparameters = [28, 4, 12, 0.8, 12, 0.001, 0]  # Default parameters
...
class ParticleSwarmOptimizer:
    def __init__(self, fun: Callable, nvars: int, lb: Union[float, List[float]] = None,
        ...
        self.options = {
            ...
            'InitialInertia': 1.2,
            'FinalInertia': 0.4,
            'LinearDecay': True,  # Flag for linear decay
            'StagnationInertiaFreeze': True ,   # Flag to freeze inertia during stagnation
    ...
    def _update_swarm(self):
        ...
        # Calculate linear decay from 1.2 to 0.4 over iterations
        if self.options['LinearDecay']:
            ...
            target_inertia = self.options['InertiaRange'][1] * (0.5 ** self.state['stall_counter'])
        
        # Only update inertia if we're not in stagnation or if stagnation freeze is disabled
        if not self.options['StagnationInertiaFreeze'] or self.state['stall_counter'] == 0:
...
def calculate_pcr(matrix, inputparameters): 
    ...
    try:
        ...
        # print(hasattr(myCST.Solver, 'SetNumberOfResultDataSamples'))  # Should be True
        ...
        # print(type(myCST.Solver))  # Compare across both scripts
        # print(dir(myCST.Solver))   # Should clearly show if the method exists
...
def pyoptimize_te(inputparameters):
    ...
    options = {
        ...
        'SelfAdjustment': 1.49,
        'SocialAdjustment': 1.49,
        'InitialInertia': 1.2,  # New parameter
        'FinalInertia': 0.4,    # New parameter
        'LinearDecay': True,    # New parameter
        'StagnationInertiaFreeze': True,  # New parameter
        'Display': 'iter',
        'MSEThreshold': inputparameters[5],
        'PrintEvery': 1
    }
