...
class Results:
    ...
    def getResultTreeItem(self, resultTreeItem: str, format: str = "complex",
                          runID: int = 0) -> tuple[np.ndarray, np.ndarray, str,
                                                  str]:               
        """Retrieve any result from the tree item (WARNING: Currently only 1D
        ...
        # Check that resultTreeItem is of type str
        if not isinstance(resultTreeItem, str):
            raise TypeError("resultTreeItem must be of type str.")
        
        # Check that format is of type str
        if not isinstance(format, str):
            raise TypeError("format must be of type str.")
        
        # Check that format presents a valid value
        validFormats = {"real", "imaginary", "complex", "magnitude", "phaseRad",
                        "phaseDeg"}
        if not format in validFormats:
            raise ValueError("format does not present a valid value.")
        
        # Check that runID is of type int
        if not isinstance(runID, int):
            raise TypeError("runID must be of type int.")
        
        # Check that runID is equal or greater than 0
        if runID < 0:
            raise ValueError("runID must be equal or greater than 0.")
        
        # Retrieve the list of runIDs in the project
        try:
            resultIDlist = self.__MWS.Resulttree.GetResultIDsFromTreeItem(
                resultTreeItem)
        except com_error as errMsg:
            # If an error occurs, raise and exception and print the error
            # message produced by CST
        ...
        # Parse the first resultID of the list to obtain its runID
        parseResult = re.search(r"3D:RunID:(\d+)", resultIDlist[0])
        # If a match is found, retrieve the runID value
        if parseResult:
            firstRunID = int(parseResult.group(1))
        # Otherwise, raise an exception
        else:
            raise RuntimeError("First resultID of resultIDlist did not match " +
                               "the expected pattern.")
        
        # Parse the last resultID of the list to obtain its runID
        parseResult = re.search(r"3D:RunID:(\d+)", resultIDlist[-1])
        # If a match is found, retrieve the runID value
        if parseResult:
            lastRunID = int(parseResult.group(1))
        # Otherwise, raise an exception
        else:
            raise RuntimeError("Last resultID of resultIDlist did not match " +
                               "the expected pattern.")

        # If the requested runID is smaller than the runID of the first
        # resultID, or if it is greater than the runID of the last resultID,
        # then it does not exist in the project. In this case, raise an
        # exception.
        if (runID < firstRunID) or (runID > lastRunID):
            raise RuntimeError("ERROR: The specified runID is not present in " +
                               "the project results.")
        
        # Fetch the contents of the result tree item for the specified runID
        try: 
            result1D = self.__MWS.Resulttree.GetResultFromTreeItem(
                resultTreeItem, f"3D:RunID:{runID}")
        except com_error as errMsg:
            # If an error occurs, raise and exception and print the error
            # message produced by CST
            raise RuntimeError("ERROR while retrieving the value of the " +
                               "resultTreeItem. CST returned: {}".format(
                                   errMsg.excepinfo[2]))
        
        # Extract the X data
        xData = np.asarray(result1D.GetArray("x"))
        xUnit = result1D.GetXUnit
        # Extract the Y data
        yDataRe = np.asarray(result1D.GetArray('yre'))
        yDataIm = np.asarray(result1D.GetArray('yim'))
        yUnit = result1D.GetYUnit
        # Combine the real and imaginary parts of yData according to the
        # specified format
        if format == "real":
            yData = yDataRe
        elif format == "imag":
            ...
            yData = yDataRe + 1j*yDataIm
        elif format == "magnitude":
            yData = np.sqrt(yDataRe**2 + yDataIm**2)
        elif format == "phaseRad":
            yData = np.arctan(yDataIm/yDataRe)
        elif format == "phaseDeg":
            yData = np.arctan(yDataIm/yDataRe)*180/np.pi
