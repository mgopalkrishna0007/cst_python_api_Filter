...
results_storage = {
    ...
    'best_solution': None,
    'all_iterations': [],
    'simulation_data': []  # New storage for simulation data
}
...
def save_simulation_data(matrix, te, freq, S, simulation_idx, inputparameters):
    """Save simulation data for each particle evaluation"""
    npix = inputparameters[4]
    dimension = inputparameters[2]
    
    # Create base directory for simulation data if it doesn't exist
    base_dir = os.path.join(os.getcwd(), "BPSO_Results")
    os.makedirs(base_dir, exist_ok=True)
    
    # Create unique folder name based on npix and dimension
    folder_name = f"npix_{npix}_dim_{dimension}mm"
    simulation_dir = os.path.join(base_dir, folder_name)
    os.makedirs(simulation_dir, exist_ok=True)
    
    # Create subfolder for this specific simulation
    sim_folder = os.path.join(simulation_dir, f"simulation_{simulation_idx:04d}")
    os.makedirs(sim_folder, exist_ok=True)
    
    # Save all data
    np.save(os.path.join(sim_folder, "pixel_configuration.npy"), matrix)
    np.save(os.path.join(sim_folder, "te_response.npy"), te)
    np.save(os.path.join(sim_folder, "frequencies.npy"), freq)
    np.save(os.path.join(sim_folder, "s_parameters.npy"), S)
    
    # Save metadata
    with open(os.path.join(sim_folder, "parameters.txt"), 'w') as f:
        f.write(f"Simulation Index: {simulation_idx}\n")
        f.write(f"Number of pixels: {npix}\n")
        f.write(f"Unit cell dimension: {dimension} mm\n")
...
def calculate_pcr(matrix, inputparameters, simulation_idx=None): 
    clear_com_cache()
    ...
    try:
        ...
        # Save simulation data if simulation index is provided
        if simulation_idx is not None:
            save_dir = save_simulation_data(matrix, te, freq, S, simulation_idx, inputparameters)
            results_storage['simulation_data'].append({
...
def create_pso_cost_wrapper(inputparameters):
    simulation_counter = 0  # Counter to track simulation runs
    
    def pso_cost_function(x_vector):
        nonlocal simulation_counter
        ...
        try:
            ...
            te, freq, S = calculate_pcr(matrix, inputparameters, simulation_counter)
            ...
            if mse <= mse_threshold:
                ...
                results_storage['below_threshold'].append({
                    ...
                    'freq': freq,
                    'save_dir': save_dir,
                    'simulation_idx': simulation_counter
                })
            ...
            print(f"Current MSE: {mse:.6f} (Simulation #{simulation_counter})")
            return mse
...
def pyoptimize_te(inputparameters):
    ...
    print(f"Total simulations run: {len(results_storage['simulation_data'])}")
...
if __name__ == "__main__":
    ...
    print(f"- All simulation data saved in BPSO_Results/npix_{inputparameters[4]}_dim_{inputparameters[2]}mm/")